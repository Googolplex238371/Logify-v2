...
def view_users():
  if not current_user.admin or current_user.teacher:
    ...
    for i in db.session.query(User).all():
      data.append([i.id,i.name,i.email,i.admin,i.teacher,i.school])
  ...
  if request.method == "POST":
    ...
    for i in data:
      ...
      if row == [""]:
        break
      if row[2] != "Student":
          user = User(teacher=row[2]=="Teacher",otp=row[3],email=row[0],password=generate_password_hash(row[3]),admin=row[2]=="Admin",name=row[1],school=row[4])
          db.session.add(user)
          db.session.commit()
    for i in data:
      print(i)
...
@views.route("/ai-suggest",methods=["GET","POST"])
@login_required
def suggested():
  if request.method == "POST":
    print(request.form.get("portfolio"))
...
def add_notes():
  if request.method == "POST":
    ...
    notes = " ".join(json.loads(request.data)["notes"].split("\n"))
    user.notes+=notes
    db.session.commit()
    return jsonify({})
